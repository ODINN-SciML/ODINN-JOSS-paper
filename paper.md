...
![Figure 1: Overview of the ODINN.jl ecosystem. \label{fig:ecosystem}](figures/odinn_ecosystem_v4.png)
...
`ODINN.jl` has been designed to address the need for a glacier model that combines the interpretability and established physical foundations of mechanistic models, based on differential equations, with the flexibility and data-assimilation capabilities of data-driven approaches [@bolibar_universal_2023]. By integrating these two paradigms, ODINN.jl enables targeted inverse methods to learn parametrizations of glacier physical processes, capturing only the unknown physics while preserving the physically grounded structure of glacier dynamics through differential equations.

While purely mechanistic and purely data-driven modelling approaches already exist in glaciology [e.g. @gagliardini_capabilities_2013; @maussion_open_2019; @rounce_global_2023; @bolibar_nonlinear_2022], there remains a clear need for flexible models capable of leveraging the wealth of available surface observations to simulate complex glacier processes such as basal sliding, creep, or calving. Existing empirical laws do not always map directly to observable quantities, making it difficult to identify or calibrate parametrizations. Approaches based on functional inversions and differentiable programming offer the necessary flexibility to derive new empirical relationships from carefully selected input proxies, providing a means to test hypotheses and discover data-driven parametrizations of poorly understood physical processes.

A robust representation of these complex and often uncertain processes is essential for improving predictions of glacier evolution and their impacts on freshwater resources and sea-level rise [@ipcc_climate_2021]. With `ODINN.jl`, we provide a unified modelling ecosystem that supports both advanced inverse methods for model calibration and efficient, modular forward simulations for large-scale glacier modelling.

These scientific challenges at the core of inverse modelling impose demanding requirements on scientific software. Inefficient, monolithic, or irreproducible code can significantly hinder progress. Thus, developing open-source, community-driven scientific software following modern development practices is vital for tackling cutting-edge research questions. The Julia programming language offers two major advantages in this regard: it solves the two-language problem by combining high-level, Python-like syntax with C-level performance [@bezanson_julia_2017], and it supports source-code differentiability, which enables the gradient computations central to inverse modelling.

With `ODINN.jl`, our goal is to build a future-proof, modular modelling framework that can evolve with emerging scientific and computational needs. By combining the clarity of mechanistic models with the adaptability of data-driven approaches, it provides a flexible and open foundation for studying glacier processes in a rapidly changing climate. The ecosystem has been developed following rigorous testing and continuous integration (CI) practices across all packages to ensure robustness and reproducibility. Ultimately, `ODINN.jl` aims to foster collaboration and accelerate advancements in the glaciological and Earth system modelling communities, bridging the gap between physical understanding and data-driven discovery.
